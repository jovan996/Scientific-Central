...
public class PaymentOfMembershipService implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		String successUrl = "http://localhost:8081/order/success/" + execution.getProcessInstanceId() + "/" + username + "/" + magazine.getId();
		
		if(magazine != null) {
			
			OrderInformationDTO order = new OrderInformationDTO(magazine.getEmail(), magazine.getMonthlyMembershipPrice(), "USD", 
							successUrl, "http://localhost:8081/order/error", "http://localhost:8081/order/fail");	
